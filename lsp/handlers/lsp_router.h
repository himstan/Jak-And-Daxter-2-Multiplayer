...
class LSPRoute {
 public:
  ...
  LSPRoute(std::function<std::optional<json>(Workspace&, json, json)> request_handler);
  ...
  std::function<std::optional<json>(Workspace&, json, json)> m_request_handler;
  /// @brief Prepares notification response bodies to be published _after_ the main handler is
  /// processed
  std::optional<std::function<std::optional<std::vector<json>>(Workspace&, json)>>
      m_post_notification_publish;
