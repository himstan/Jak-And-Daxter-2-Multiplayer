...
void write_base_header(uint8_t* output, uint8_t kind) {
  ...
  multiplayer::wire::store_u16_le(output + 4, kProtocolVersion);
  output[6] = kind;
  output[7] = 0;
}
...
bool valid_base_header(const uint8_t* data, size_t size, uint8_t kind) {
  return data && size >= kBaseHeaderSize &&
         ...
         multiplayer::wire::load_u16_le(data + 4) == kProtocolVersion && data[6] == kind;
}
...
bool MultiplayerSecurity::make_server_hello(MultiplayerDatagram& output) const {
  if (!m_host || m_encoded_token.empty()) {
    return false;
  }
  output.size = kBaseHeaderSize + m_session_id.size() + m_server_nonce.size();
...
void MultiplayerSecurity::derive_session_keys(int local_role) {
  ...
  std::copy_n(receive_nonce.begin(), m_receive_nonce_prefix.size(),
              m_receive_nonce_prefix.begin());
...
SecurityReceiveResult MultiplayerSecurity::receive(int local_role,
  ...
  if (!m_authenticated && local_role == 1 &&
      valid_base_header(data, size, kServerHello) &&
      size == kBaseHeaderSize + m_session_id.size() + m_server_nonce.size()) {
    memcpy(m_session_id.data(), data + kBaseHeaderSize, m_session_id.size());
    memcpy(m_server_nonce.data(), data + kBaseHeaderSize + m_session_id.size(),
           m_server_nonce.size());
    randombytes_buf(m_client_nonce.data(), m_client_nonce.size());
    std::array<uint8_t, kKeySize> proof = {};
    make_proof("client-proof", proof);
    result.response.size = kBaseHeaderSize + m_session_id.size() + m_client_nonce.size() +
                           proof.size();
  ...
  if (!m_authenticated && local_role == 0 &&
      valid_base_header(data, size, kClientProof) &&
      size == kBaseHeaderSize + m_session_id.size() + m_client_nonce.size() + kKeySize &&
      sodium_memcmp(data + kBaseHeaderSize, m_session_id.data(), m_session_id.size()) == 0) {
    const size_t client_nonce_offset = kBaseHeaderSize + m_session_id.size();
    memcpy(m_client_nonce.data(), data + client_nonce_offset, m_client_nonce.size());
    std::array<uint8_t, kKeySize> expected = {};
    make_proof("client-proof", expected);
    const uint8_t* supplied = data + client_nonce_offset + m_client_nonce.size();
    if (sodium_memcmp(supplied, expected.data(), expected.size()) != 0) {
      sodium_memzero(expected.data(), expected.size());
      return result;
    }
    sodium_memzero(expected.data(), expected.size());
    std::array<uint8_t, kKeySize> server_proof = {};
    make_proof("server-proof", server_proof);
    result.response.size = kBaseHeaderSize + m_session_id.size() + server_proof.size();
    write_base_header(result.response.bytes.data(), kServerProof);
    memcpy(result.response.bytes.data() + kBaseHeaderSize, m_session_id.data(),
           m_session_id.size());
    memcpy(result.response.bytes.data() + kBaseHeaderSize + m_session_id.size(),
           server_proof.data(), server_proof.size());
  ...
  if (!m_authenticated && local_role == 1 &&
      ...
      size == kBaseHeaderSize + m_session_id.size() + kKeySize &&
      sodium_memcmp(data + kBaseHeaderSize, m_session_id.data(), m_session_id.size()) == 0) {
    std::array<uint8_t, kKeySize> expected = {};
    make_proof("server-proof", expected);
    const uint8_t* supplied = data + kBaseHeaderSize + m_session_id.size();
    if (sodium_memcmp(supplied, expected.data(), expected.size()) != 0) {
      sodium_memzero(expected.data(), expected.size());
      return result;
    }
  ...
  if (decrypted_size < sizeof(PacketHeader) ||
      result.plaintext.bytes[0] != data[9]) {
    result.plaintext.size = 0;
    return result;
  }
...
bool MultiplayerSecurity::seal(int local_role,
  ...
  if (!m_authenticated || !plaintext || plaintext_size == 0 ||
      packet_type >= PacketType::COUNT ||
  ...
  multiplayer::wire::store_u16_le(output.bytes.data() + 10,
                                  static_cast<uint16_t>(plaintext_size));
